...
<!--
	...
	var PageUtil = {
		...
		del : function(id) {
			if(confirm("是否删除?")) {
				window.location.href = "<c:url value="/admin/city/deleteCity" />?ids=" + id;
			}
		},
		changeStatus : function(id){
			if(confirm("是否更改?")) {
				window.location.href = "<c:url value="/admin/city/changeStatus" />?id=" + id ;
			}
		},
		delMulti : function() {
			var ids = [];
...
<div class="main_right"  id="main_col1"  style="overflow:hidden;">
	<div class="hangy_qh2">
		<ul class="TabbedPanelsTabGroup">           
			<li class="TabbedPanelsTab TabbedPanelsTabSelected" >城市</li>
		</ul>
		...
		<div class="TabbedPanelsContent"  id="main_col2" >
			...
			<div class="yhtz">
				<a href="<c:url value="/admin/city/edit/add/0" />" class="lvse_btn">新建</a>
				...
				<c:forEach items="${list}" var="city">
					<tr>
						...
						<td>
							...
							<a class="btn" href="javascript:PageUtil.del(${city.id});void 0;">删除</a>&nbsp;&nbsp;&nbsp;&nbsp;
							<c:choose>
								<c:when test="${city.status == 1}">
									<a class="btn" href="javascript:PageUtil.changeStatus(${city.id});void 0;">使下线</a>
								</c:when>
								<c:when test="${city.status == 0}">
									<a class="btn" href="javascript:PageUtil.changeStatus(${city.id});void 0;">使上线</a>
								</c:when>
							</c:choose>
						</td>
					</tr>
			...
			<div class="page">
				<a href="<c:url value="/admin/city/list?page=1" />">首页</a>
				<c:if test="${currentPage > 1}">
		       		<a href="<c:url value="/admin/city/list?page=" />${currentPage-1}">上一页</a>
		       	</c:if>
				<c:forEach var="i" begin="1" end="${totalPage}" varStatus="status">
					<c:if test="${status.index == currentPage}">
			       		<a href="<c:url value="/admin/city/list?page=" />${status.index}" class="hover">${status.index}</a>
			       	</c:if>
			       	<c:if test="${status.index != currentPage}">
			       		<a href="<c:url value="/admin/city/list?page=" />${status.index}">${status.index}</a>
			       	</c:if>
				</c:forEach>
				<c:if test="${currentPage < totalPage}">
					<a href="<c:url value="/admin/city/list?page=" />${currentPage+1}">下一页</a>
				</c:if>
				<a href="<c:url value="/admin/city/list?page=" />${currentPage+1}">尾页</a>
